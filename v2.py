...
while True:
    ...
    try:
        ...
        # 에러는 안나는데 빈 창만 뜨고 다음으로 안넘어간다. 어노잉 오렌지 창은 안떴음
        # le_cropped = frame[[region_left]]
        # cv2.imshow("le_cropped", le_cropped)
        ...
        cv2.fillPoly(le_mask, [region_left], (255, 255, 255))  # 검은 배경 흰 다각형. 원하는 부분이 흰색
        # cv2.imshow('le_mask', le_mask)
        le_frame = cv2.bitwise_or(frame, frame, mask=le_mask)
        # cv2.imshow('le_frame', le_frame)
        # left_eye_frame = resize(le_frame, width=10000 )
